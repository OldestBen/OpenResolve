...
if [[ -f "$ENV_FILE" ]]; then
  ...
  # Values are double-quoted so passwords with spaces or shell-special chars
  # ($, !, &, etc.) are stored correctly and read back by Docker Compose.
  cat > "$ENV_FILE" <<EOF
...
# On a fresh config (.env was just created), tear down any containers and data
# volume from a previous partial/test run. Without this, a stale DB seeded with
# a different password would cause login failures on the very first use.
if [[ "$FRESH_CONFIG" == "true" ]]; then
  info "Fresh configuration — clearing any stale data from previous runs..."
